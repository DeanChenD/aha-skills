...
- 日期：2026-05-20
- 状态：待评审
- 目的：在完全抛弃当前实现的前提下，给出从零重写所需的完整蓝图

---

## 1. 背景

当前 `aha-skills` 实现已经偏离初衷。原本想要的是「把易逝的认知瞬间留下并持续生长」的轻量 skill 集合，结果变成了：

- `_lib/aha_md.py`：frontmatter 解析、section finder、原子写、flock、workspace anchor、schema_version、period_range……
- 每个 skill 一个 `*_md.py` CLI（capture/update/refine/discuss/checkin/log/list/scan/review……）
- 一整套测试套件 + `scripts/run_tests.py`
- 跨 host 写冲突检测（mtime + flock）、`[SILENT]` 协议、conflict 文件扫描、shell 注入面专项防御
- `references/` 子流程文件、按天聚合的 `log-YYYY-MM-DD.md`、按周/月生成的 `review-*.md`

这些都是为了一条已经不再成立的公约服务的：「state mutation 走 deterministic CLI，CLI 是写入边界、测试边界、shell 注入边界」。

新的设计公约把写入交回给 Agent + 通用工具（`Read` / `Edit` / `Write` / `Glob` / `Grep` / `Bash`），上述基础设施全部失去存在理由。本设计据此重写。
...
> 注：原计划中的 `reflect`（跨 skill 模式挖掘）暂不实现，目前对用户没有足够价值；原 `daily` 的 log / check-in / review 三件事也一并砍掉，todo 收敛为纯任务管理。
...
3. **Agent 自行读取和编辑 `.md` 文件**。Agent 尽可能使用已有的合适工具读写 md 文件，尽可能不提供额外脚本，不重复造轮子。
...
3. **YAGNI**：不为假设中的未来需求提前设计。暂不实现多用户、权限、同步、插件系统等能力。只有真实、明确出现的需求，才允许进入设计。
...
- ~~`_lib/aha_md.py`~~：没有需要共享的 utility
- ~~`skills/<name>/scripts/*_md.py`~~：Edit / Write / Bash 已经够用
- ~~`skills/<name>/tests/test_*.py`~~：没有可测代码
- ~~`skills/<name>/references/*.md`~~：SKILL.md 自身写清楚就好
- ~~`scripts/run_tests.py` / `Makefile`~~：没有测试可跑
- ~~`aha-data/.manifest.json`~~：没有需要锚定的 schema
- ~~`reflect/`~~：放弃
- ~~daily 的 `logs/` / `check-ins/` / `reviews/`~~：放弃
...
一个 skill 的所有记录平铺在该子目录里，不再分 `idea-md/` `sessions/` 等二级结构。Agent 在第一次写入时若目录不存在，用 `mkdir -p` 创建。
...
- 各 skill 可按需追加字段（详见 §7）

**正文**，公共字段：

- `## Raw`：用户原始输入。**永不被覆盖、永不被改写**。

各 skill 在 Raw 之外按需追加 section（详见 §7）。
...
```
2026-05-20T15:30:00+08:00
```

Agent 用 Bash 取当前时间（前提：用户在 +08:00 时区，作者目前所在）：
...
具体哪些 section 适用本模式由各 skill 显式列出（详见 §7）。Agent 修改这些 section 之前，必须先把当前内容追加到对应 Log 的尾部，再用新内容覆盖。这一规则由 SKILL.md 的硬约束保障，不靠代码（详见 §8）。
...
- vs `todo`：todo 是有 due 的具体任务；idea 是在没有 due / 形态未定的探索
...
> 关键变化：旧实现把每次 discussion 单独写到 `aha-workspace/dao/sessions/<id>-session-NNN.md`。新设计**直接 append 到主文件的 `## Discussions`** 里，不再有 sessions 子目录。一份 dao 永远是一份文件。

**典型工作流**：

1. 用户说"我刚刚悟到 X"
2. Agent 创建 dao 文件，把 X 写进 `## Raw`
3. **除非用户明确说"先放着"**，Agent 立刻起草一版 `## Summary`（比 Raw 更紧、更清晰，不要加新主张、不说教）
...
- vs `dao`：见 §7.2 的判定口径——能不能脱离实践域说出来。**人换了 tip 还能用，人变了 dao 才作用**
...
> 关键设计取舍：**不单独保留 Domain / Reuse Log / Generalization 三个 section**。Domain 融进 Summary 的开头（"在 PR 评审场景下，先看 test……"自然就锚定了 domain）。复用次数和泛化推广不在文件里追踪，谁要用谁口头说，需要的时候 Agent 用 Grep 一搜就有。
...
> 关键变化：todo 不保留 daily 的 logs / check-ins / reviews 三件事。任务文件就是任务文件，没有日志、没有阶段对话记录、没有周/月汇总。
...
不需要的：`Install note`、`[SILENT]` 段落、`Scheduled Review` 段落、`scripts/` 引用——v0 不做 cron / scheduler 集成。
...
## 11. 不在本设计里的（明确 out-of-scope）

- `reflect` skill / 跨源汇总文件 / 周月复盘
- 多机 / 多用户 / cron 同步
- `[SILENT]` 协议
...
- 测试套件
- frontmatter 里的 tags / priority / next_review_at / refine_count / reuse_count / domain（已被显式剔除）

如果未来某一项变成真实痛点，作为单独的 spec 重新讨论。
...
1. 在新分支上 `rm -rf skills/_lib skills/idea skills/dao skills/daily skills/reflect scripts Makefile`，保留旧 `README.md` / `README.en.md` 作为对照（也可 `git rm`，反正历史还在）
2. **保留** `docs/superpowers/specs/2026-05-20-aha-skills-redesign.md`（本文档）作为重写过程中的事实蓝图
3. 按本文档 §7 的描述新建 4 个 SKILL.md：`skills/{idea,dao,tip,todo}/SKILL.md`
4. 按本文档 §2-§4 重写 `README.md`：哲学 / 公约 / 约束 / 该用哪个 skill 的路由表
5. 不再写任何 Python 文件、不再写任何测试

每个 SKILL.md 的写作可以以本设计 §7 对应小节为底稿，扩出 §8 列出的所有区块。

---

## 13. 开放问题（写完留作 follow-up）
